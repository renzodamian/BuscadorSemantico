...
class InicioView(TemplateView):
    ...
    def post(self, request):
        ...
        if formulario.is_valid():
            ...
            # llama clase
            token = Semantico()
            datos, ent, spacyText,datostype= token.consultaVirutoso(text)
            texto_inicial = token.textoHtml(text, ent)
            # elimina datos dupliados de lista
            datos = OrderedDict((tuple(x), x) for x in datos).values()
            args = {"datos": datos, "formulario": self.formulario, "texto_inicial": texto_inicial,"spacyText":spacyText,"datatype":datostype}
        return render(request, self.plantilla, args)
...
class Semantico():
    ...
    def limpiarDatos(self,palabra):
        ...
        palabra = palabra.replace('Alianza_PAIS','AP')
        palabra = palabra.replace('Odebrecht','ODEBRECHT')
